...
2. **アプリケーションで設定**
   ...
   - モデル: **GPT-5.1 Mini** または **GPT-4.1 Mini**（最新のバランスモデル）
   - APIキー: `sk-...` （あなたのAPIキー）
   - 「接続をテスト」をクリック

   > **推奨モデル**:
   > - **OpenAI GPT**: GPT-5.1 Mini（最新・高速）、GPT-4.1 Mini（バランス型）、o1-mini（推論タスク）
   > - **Anthropic Claude**: Claude Haiku 4.5（高速）、Claude Sonnet 4.5（バランス型）、Claude Opus 4（最高品質）
   > - **Google Gemini**: Gemini 2.5 Flash（高速）、Gemini 2.5 Pro（高品質）、Gemini 3.0 Pro (Preview)（最新）
...
3. **複数のAIプロバイダーをテスト**
   - **OpenAI**: マルチモーダル（画像・音声）と高度推論に強い。GPT-5.1系が最新。
   - **Claude**: 長文処理（最大20万トークン）と整合性チェックに強い。Claude 4/4.5ファミリー。
   - **Gemini**: 超長文コンテキスト（最大200万トークン）とマルチモーダルに強い。Gemini 3.0 Pro (Preview)が最新。
