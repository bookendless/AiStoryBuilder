...
// 共通の改行指示テンプレート
const NEWLINE_INSTRUCTIONS = `【改行の指示】
- 会話の前後で改行する
- 場面転換時に改行する
- 段落の区切りで改行する
- 長い文章は読みやすく適度に改行する`;
...
2. **文字数**: 各章3000-6000文字程度で執筆してください
...
- revisedText内の改行は必ず\\\\nで表現してください（例: "段落1\\\\n\\\\n段落2"）
- 元の文章の内容をすべて保持・加筆しつつ、指摘された弱点を克服した文章にしてください
- 目標文字数は{currentLength}文字以上です。短縮・要約は禁止し、加筆で改善してください
...
【出力制約（厳守）】
- **執筆した本文のみを出力してください**
- 分析プロセス、思考過程、ステップの説明は一切出力しないでください
- 「ステップ1」「分析結果」などの見出しは出力しないでください
- 1000〜1500文字程度で自然な流れで書いてください
- 上記の文体設定を厳密に守り、一貫性のある文章を執筆してください
- 章の雰囲気や登場キャラクターの性格を維持しながら執筆してください`,

  fixCharacterInconsistencies: `以下の草案に含まれるキャラクター情報の矛盾や不一致を修正してください。
...
【修正指示】
1. **キャラクター設定の確認**: 草案に登場するキャラクターの性格、外見、口調、背景が、プロジェクトの設定と一致しているか確認してください。
2. **矛盾の修正**:
   - 性格が矛盾している箇所を修正（例：内向的な人が突然饒舌になる、冷静な性格のはずが感情的）
   - 外見が矛盾している箇所を修正（例：黒髪と記載されているのに金髪と描写される、身長や体型の不一致）
   - 口調が矛盾している箇所を修正（例：丁寧語を使うはずが乱暴な言葉遣い、方言の有無）
   - 背景設定が矛盾している箇所を修正（例：貧しい出身のはずが裕福な描写、職業や立場の不一致）
3. **一貫性の確保**: キャラクターの行動や発言が、設定された性格や背景と一貫するように修正してください。
4. **文章の品質維持**: 修正箇所以外の表現や流れはできるだけ保持し、自然な文章にしてください。
5. **文字数の維持**: 現在の文字数（{currentLength}文字）を可能な限り維持してください。

【出力制約（厳守）】
- **修正された草案全文のみを出力してください**
- 説明文、コメント、修正箇所の説明は一切出力しないでください
- 「以下のように修正しました」などの前置きは不要です
- 草案本文のみを出力してください
