...
以下のJSON形式で出力してください：
{
  ...
  "weaknessDetails": [
    { "point": "改善点1と同じ内容", "quote": "該当箇所を評価対象テキストから一字一句そのまま抜き出した引用（80文字以内）" }
  ],
...
${JSON_OUTPUT_RULES}
- weaknessDetails は weaknesses と同じ内容を、該当箇所の引用つきで並べたもの（引用できない指摘は quote を省く）
- quote は評価対象テキストに実在する文字列だけを使う（存在しない引用を作らない）
- 修辞・美文の多さを加点しない。過剰な比喩・修飾はむしろ減点対象とする`,
...
以下のJSON形式で出力してください：
{
  ...
  "weaknessDetails": [
    { "point": "物足りなかった点1と同じ内容", "quote": "該当箇所を評価対象テキストから一字一句そのまま抜き出した引用（80文字以内）" }
  ],
