...
指摘が特定の登場人物または用語に関するものであれば targetType と targetName を付けてください（設定書へ補記する導線に使います）。どちらとも言えない指摘では省略してください。
  ...
  "issues": [
    {
      ...
      "suggestion": "修正案（任意）",
      "targetType": "character | term のいずれか（特定の登場人物・用語に関する指摘のときだけ。任意）",
      "targetName": "対象の登場人物名または用語名。設定台帳に書かれている表記をそのまま使う（任意）"
    }
  ]
}`;
}
