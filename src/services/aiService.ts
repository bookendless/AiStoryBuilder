    ...
    try {
      ...
      const requestBody: OpenAIRequestBody = {
        ...
        messages: [
          {
            role: 'system',
            content: request.systemPrompt || SYSTEM_PROMPT,
          },
          {
            role: 'user',
            content: userContent,
...
（注：あらすじは参考情報としてのみ使用し、他の設定と矛盾する場合は他の設定を優先してください）`;
  ...
  private buildStyleDetails(variables: Record<string, string>): string {
    ...
    if (!perspective && !formality && !rhythm && !metaphor && !dialogue && !emotion && !tone && !styleSample) {
      return '';
    }
    ...
    // 文体見本（few-shot）。選択肢ラベルより直接的に文体を伝えるため最後に置く
    if (styleSample) {
      details.push('');
      details.push('【文体見本（最重要・この文章の雰囲気・文体・語り口に合わせて執筆する）】');
      details.push('---');
      details.push(styleSample);
      details.push('---');
      details.push('※見本の内容（出来事・人物）を流用せず、文体・リズム・語彙の傾向だけを真似てください。');
    }
