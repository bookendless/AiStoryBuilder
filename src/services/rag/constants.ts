...
/**
 * 【予算とCAPの関係】一見矛盾して見えるので明記する。
 *
 * - `getInputCharBudget(settings)` ≒ 8500 … **可変データに配れる文字数**の予算。
 *   検索・整形の段階で「どこまで詰めるか」を決めるために使う。
 * - `DRAFT_PROMPT_CAP` = 30000 … **組み立て後のプロンプト全体**のサニタイズ上限。
 *   逐語で入るもの（草案全文・前章末尾・文体見本・カスタム指示）とテンプレートを
 *   足すと 8500 を大きく超えるため、CAP はそれを飲み込める値にしてある。
 *
 * つまり 8500 は「集めてよい量」、30000 は「送ってよい量」で、比較する数値ではない。
 * 8500 を CAP に合わせて引き上げると、収集量が増えてトークン課金が黙って増える。
 */
