...
const isValidRelationshipType = (t: unknown): t is CharacterRelationship['type'] =>
  typeof t === 'string' && t in relationshipTypes;

// 不正・未知タイプは 'other' にフォールバックして必ず定義オブジェクトを返す
const getRelationshipType = (t: unknown) =>
  relationshipTypes[isValidRelationshipType(t) ? t : 'other'];
...
export const RelationshipDiagram: React.FC<RelationshipDiagramProps> = ({ isOpen, onClose }) => {
  ...
  const flowChartLayout = useMemo<{ nodes: FlowChartNode[]; edges: FlowChartEdge[]; svgWidth: number; svgHeight: number } | null>(() => {
    ...
    const edges: FlowChartEdge[] = relationships.map((rel, idx) => {
      ...
      const relationshipType = getRelationshipType(rel.type);
  ...
  const getProjectContext = (): string => {
    ...
    if (relationships.length > 0) {
      ...
      relationships.forEach(rel => {
        ...
        context += `- ${fromChar?.name || '不明'} → ${toChar?.name || '不明'}: ${getRelationshipType(rel.type).label} (強度: ${rel.strength}/5)`;
        if (rel.description) {
          context += `\n  説明: ${rel.description}`;
        }
  ...
  const handleInferRelationships = async () => {
    ...
    try {
      ...
      const prompt = buildRelationshipInferPrompt(projectContext, characterNames);
          ...
          const filteredRelationships = inferredRelationships
            .map(rel => {
              ...
              return {
                from: fromChar.id,
                to: toChar.id,
                type: isValidRelationshipType(rel.type) ? rel.type : 'other',
  ...
  const handleSuggestRelationships = async () => {
    ...
    try {
      ...
      const prompt = buildRelationshipSuggestPrompt(projectContext, characterNames);
  ...
  const handleCheckConsistency = async () => {
    ...
    try {
      ...
      if (response.content) {
        ...
        showSuccess('関係性の整合性チェックが完了しました');
      }
    } catch (error) {
  ...
  const handleGenerateDescription = async () => {
    ...
    try {
      ...
      const prompt = buildRelationshipDescriptionPrompt(
        projectContext,
        fromChar?.name || '',
        fromChar?.role || '',
        toChar?.name || '',
        toChar?.role || '',
        getRelationshipType(formData.type).label,
        formData.strength || 3
      );
              ...
              relationships.length === 0 ? (
                ...
                <div className="grid gap-4">
                  {relationships.map((rel) => {
                    const typeInfo = getRelationshipType(rel.type);
