...
export const TimelineViewer: React.FC<TimelineViewerProps> = ({ isOpen, onClose }) => {
  ...
  const handleExtractEvents = async () => {
    ...
    try {
      ...
      const prompt = buildTimelineExtractEventsPrompt(
        projectContext,
        JSON.stringify(timeline.map(e => e.title), null, 2)
      );
  ...
  const handleGenerateDescription = async () => {
    ...
    try {
      ...
      const prompt = buildTimelineDescriptionPrompt(projectContext, formData.title, formData.date, formData.category);
  ...
  const handleCheckConsistency = async () => {
    ...
    try {
      ...
      const prompt = buildTimelineConsistencyCheckPrompt(projectContext, timelineText);
  ...
  const handleSuggestEvents = async () => {
    ...
    try {
      ...
      const prompt = buildTimelineSuggestPrompt(projectContext, timelineText);
